...
import { mockRequest, mockRequestWithError } from './util/mockGitHubRequest';
describe('handleAssignees', () => {
  ...
  it('should fail adding assignees', async () => {
    const github = mockRequestWithError(
      'post',
      `/repos/${owner}/${repo}/issues/${prNumber}/assignees`,
      'Resource not accessible by integration'
    );

    const response = handleAssignees(client, {
      owner,
      repo,
      prNumber,
      matchingRules: [rule],
    ...
    await expect(response).rejects.toMatchInlineSnapshot(
      '[HttpError: request to https://api.github.com/repos/gagoar/example_repo/issues/1/assignees failed, reason: Resource not accessible by integration]'
    );
